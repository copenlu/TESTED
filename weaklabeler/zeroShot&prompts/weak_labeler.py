...
from weaklabeler.tools.utils import list_segmentor, insert_scheme
...
class WeakLabeler:
	...
	def __init__(self, model_type: str = '', labeler_type: str = 'zero', dataset_path: str = '', config_path: str = '',\
		 parallelize: bool = True, cuda_index: int = 0, hypothesis_separator: str = None):
		...
		WeakLabeler.labeler_metadata['hypothesis_separator'] = hypothesis_separator

		WeakLabeler.labeler_metadata = dotdict(WeakLabeler.labeler_metadata)


		self.hypothesis_separator = hypothesis_separator
	...
	def weak_labeler_instance(self, user_metas: List = [])-> List:
		bulk_result = None
		...
		try:
			...
			if 'mlm' in env.labeler_metadata.labeler_type.lower():
				...
				mlm_labels = user_metas[0]['labels']


				# Numworkers doesnt really matter as long as threadcout is similar
				results_raw =  env.pipeline(tweet_texts, targets = mlm_labels, num_workers = 0, batch_size = 16)
				...
				topics, hypothesis_template = zero_meta_dict['Topics'],zero_meta_dict['Hypothesis']


				if env.labeler_metadata.hypothesis_separator is None:
					results_raw = env.pipeline(tweet_texts, topics, hypothesis_template=hypothesis_template, num_workers = 0)
				else:
					results_raw =[]
					for tweet_text in tweet_texts:

						hypothesis, premise = tweet_text.split(env.labeler_metadata.hypothesis_separator)
						hypothesis = hypothesis.strip() + " "
						topics_aug = [f"{premise.strip()} {topic}" for topic in topics]

						result_raw = env.pipeline(hypothesis.strip(), topics_aug, hypothesis_template=hypothesis_template, num_workers = 0)
						results_raw.append(result_raw)


				results_processed = self.post_porcess_zero(results_raw)
			...
			for index, user_meta in enumerate(user_metas):
				...
				if 'mlm' in env.labeler_metadata.labeler_type:
					if user_meta['class'] not in results[user_id][tweet_id]:
						...
						per_class_processor['labels'] = mlm_labels
